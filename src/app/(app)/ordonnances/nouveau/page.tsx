...
function NouvelleOrdonnancePageInner() {
    ...
    used.forEach((m) => {
      m.contre_indications?.forEach((ci) => {
        if (patient.allergies.some((a) => a.toLowerCase() === ci.toLowerCase())) {
          found.push({ tone: "danger", message: `${t("allergy_to")} ${ci} — ${t("contraindication_for")} ${m.nom_commercial}` });
        }
      });
    });
    ...
    for (let i = 0; i < used.length; i++) {
      for (let j = i + 1; j < used.length; j++) {
        if (used[i].interactions?.includes(used[j].id) || used[j].interactions?.includes(used[i].id)) {
          found.push({ tone: "warning", message: `${t("possible_interaction")}: ${used[i].nom_commercial} + ${used[j].nom_commercial}` });
        }
      }
    }
    return found;
  }, [patient, lignes, medicaments, t]);
  ...
  function submit() {
    if (!patientId || !medecinId || lignes.length === 0) {
      pushToast({ title: t("ordonnance_incomplete"), description: t("ordonnance_incomplete_desc"), tone: "warning" });
      return;
    }
    ...
    pushToast({ title: t("ordonnance_created"), tone: "success" });
  ...
  return (
    <div className="space-y-4">
      <Link href={patientId ? `/patients/${patientId}` : "/ordonnances"} className="inline-flex items-center gap-1 text-[13px] text-ink-500 hover:text-ink-800 cursor-pointer">
        <ChevronLeft className="dir-icon h-4 w-4" /> {t("back")}
      </Link>
      <SectionHeader title={t("new_ordonnance_btn")} description={patient ? `${t("for_patient")} ${patient.prenom} ${patient.nom}` : undefined} />
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 space-y-4">
            ...
            <Card padding="md" className="border-danger/40 bg-red-50/50">
              <div className="flex items-start gap-2">
                ...
                <div className="space-y-1 min-w-0">
                  <div className="text-[13px] font-semibold text-danger">{t("clinical_alerts")}</div>
                  {alerts.map((a, i) => (
          ...
          <Card padding="md">
            <div className="text-[13px] font-semibold text-ink-800 mb-2">{t("prescribed_medicaments")}</div>
            {lignes.length === 0 ? (
              <EmptyState
                icon={<Search className="h-6 w-6" />}
                title={t("no_medicament_added")}
                description={t("no_medicament_added_desc")}
              />
            ) : (
            ...
            <div className="mt-3">
              <Input
                placeholder={t("search_medicament_placeholder")}
                value={medQuery}
              ...
              <div className="mt-2 grid sm:grid-cols-2 gap-1.5">
                {suggestions.map((m) => (
                  <button
                    ...
                    className="flex items-center gap-2 rounded-lg border border-line px-2.5 py-2 text-left hover:bg-brand-50 cursor-pointer min-w-0"
                  >
                    <div className="flex-1 min-w-0">
          ...
          <Card padding="md">
            <Field label={t("remarques")}>
              <Textarea value={remarques} onChange={(e) => setRemarques(e.target.value)} rows={2} placeholder={t("remarques_placeholder")} />
            </Field>
          </Card>
        </div>

        <Card padding="md" className="space-y-2">
          <div className="text-[13px] font-semibold text-ink-800">{t("ordonnance_preview")}</div>
          <div className="text-[13px] text-ink-500">
            {t("ordonnance_preview_desc")}
          </div>
          {patient && (
            <div className="rounded-xl border border-line p-3 space-y-1 text-[12px]">
              <div><span className="text-ink-500">{t("patient")}:</span> <span className="font-medium">{patient.prenom} {patient.nom}</span></div>
              <div><span className="text-ink-500">{t("patient_age")}:</span> {new Date().getFullYear() - new Date(patient.date_naissance).getFullYear()} {t("age_years")}</div>
              {patient.allergies.length > 0 && (
                <div>
                  <span className="text-ink-500">{t("allergies")}:</span>{" "}
      ...
      <div className="sticky bottom-0 bg-surface-muted/90 backdrop-blur-sm py-3 flex items-center justify-end gap-2">
        ...
        <Button variant="primary" leftIcon={<Save className="h-4 w-4" />} onClick={submit}>{t("issue_ordonnance")}</Button>
      </div>
    </div>
  );
}
