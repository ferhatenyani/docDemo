  ...
  return (
    <div className="space-y-5">
      <Link href="/rendez-vous" className="inline-flex items-center gap-1 text-[12px] text-ink-500 hover:text-ink-800 cursor-pointer transition-colors">
        <ChevronLeft className="h-3.5 w-3.5" /> Rendez-vous
      </Link>
      <SectionHeader eyebrow="File d'attente" title="Salle d'attente" description={`${waiting.length} patient${waiting.length > 1 ? "s" : ""} en attente aujourd'hui`} />

      <div className="grid lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-lg border border-line shadow-xs">
          <div className="px-4 py-3 border-b border-line flex items-center justify-between">
            <div className="text-[13px] font-semibold text-ink-800 tracking-crisp">En attente</div>
            <Badge tone="info" dot>{waiting.length}</Badge>
          </div>
          {waiting.length === 0 ? (
            <EmptyState icon={<User2 className="h-5 w-5" />} title="Personne en salle d'attente" description="Tous les patients ont été vus." />
          ) : (
        ...
        <div className="bg-white rounded-lg border border-line shadow-xs">
          <div className="px-4 py-3 border-b border-line flex items-center justify-between">
            <div className="text-[13px] font-semibold text-ink-800 tracking-crisp">Terminés</div>
            <Badge tone="success" dot>{done.length}</Badge>
          </div>
          {done.length === 0 ? (
            <EmptyState icon={<Check className="h-5 w-5" />} title="Aucune consultation terminée" />
          ) : (
            <div className="divide-y divide-line">
              {done.map((r) => {
                const p = patients.find((x) => x.id === r.patient_id);
