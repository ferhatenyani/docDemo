...
function RendezVousPageInner() {
  ...
  function statutLabel(s: StatutRDV) {
    return s === "CONFIRME" ? t("status_confirmed")
      : s === "EN_ATTENTE" ? t("status_pending")
      : s === "TERMINE" ? t("status_done")
      : s === "ABSENT" ? t("status_absent")
      : t("status_cancelled");
  }
  ...
  return (
    <div className="space-y-5">
      <SectionHeader
        eyebrow={t("agenda")}
        title={t("rdv_title")}
        description={t("rdv_desc")}
        actions={
          <>
            <Link href="/rendez-vous/salle-attente">
              <Button variant="secondary" leftIcon={<Users className="h-3.5 w-3.5" />}>{t("waiting_room")}</Button>
            </Link>
      ...
      <div className="flex flex-wrap items-center gap-3 justify-between">
        <Tabs
          variant="segmented"
          tabs={[
            { key: "day", label: t("view_day") },
            { key: "week", label: t("view_week") },
            { key: "month", label: t("view_month") },
          ]}
          active={view}
          onChange={(k) => setView(k as any)}
        />
        <div className="flex flex-wrap items-center gap-1.5">
          <Chip active={medecinFilter === "Tous"} onClick={() => setMedecinFilter("Tous")}>{t("all_doctors")}</Chip>
          {medecins.map((m) => (
            ...
            <div className="bg-white rounded-lg border border-line shadow-xs">
              <div className="px-4 py-3 border-b border-line flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="text-[14px] font-semibold text-ink-900 tracking-crisp truncate">{formatDate(selected)}</div>
                  <div className="text-[11px] text-ink-500">{dayRdv.length} {t("rdv_count")}</div>
                </div>
                <div className="flex items-center gap-0.5 shrink-0">
                  ...
                  ><ChevronLeft className="h-3.5 w-3.5 dir-icon" /></button>
                  <button
                  ...
                  ><ChevronRight className="h-3.5 w-3.5 dir-icon" /></button>
                </div>
              </div>
              ...
              {dayRdv.length === 0 ? (
                <EmptyState
                  ...
                  title={t("no_rdv_today")}
                  description={t("free_day")}
                  action={<Button variant="dark" onClick={() => setOpenForm(true)}>{t("rdv_new")}</Button>}
                />
              ) : (
                    ...
                    return (
                      <div key={r.id} className="px-4 py-2.5 flex items-center gap-3 hover:bg-ink-50/60 transition-colors">
                        <div className="w-14 shrink-0 text-center border-e border-line pe-3">
                          ...
                          <div className="text-[10px] text-ink-500">{r.duree_minutes}{t("min_short")}</div>
                        </div>
                        {p ? (
                          ...
                          <div className="flex-1 text-[12px] text-ink-500">{t("patient_deleted_short")}</div>
                        )}
                        <div className="hidden md:block text-[11px] text-ink-500 shrink-0">
                          {m ? `Dr. ${m.nom}` : ""}
                        </div>
                        <div className="shrink-0"><Badge tone={statutTone(r.statut)} dot>{statutLabel(r.statut)}</Badge></div>
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div className={"text-[10px] " + (isSel ? "text-white/70" : "text-ink-500")}>
                          {dObj.toLocaleDateString(locale === "ar" ? "ar-DZ" : "fr-DZ", { weekday: "short" })}
                        </div>
                      ...
                      <div className="space-y-1">
                        ...
                        {list.length > 3 && (
                          <div className={"text-[10px] " + (isSel ? "text-white/60" : "text-ink-500")}>+{list.length - 3} {t("and_more")}</div>
                        )}
                        {list.length === 0 && (
          ...
          {view === "month" && (
            <div className="bg-white rounded-lg border border-line shadow-xs">
              <div className="px-4 py-3 border-b border-line text-[12px] text-ink-500">
                {t("total_this_month")}: <span className="font-semibold text-ink-800 tabular">{rdvFiltered.filter((r) => r.date.startsWith(selected.slice(0, 7))).length}</span> {t("rdv_count")}
              </div>
                      ...
                      <div key={r.id} className="rounded-md border border-line p-2.5 flex items-center gap-3 bg-white hover:border-line-strong transition-colors">
                        <div className="text-center shrink-0">
                          ...
                          <div className="text-[10px] text-ink-500">
                            {new Date(r.date + "T00:00:00").toLocaleDateString(locale === "ar" ? "ar-DZ" : "fr-DZ", { day: "2-digit", month: "short" })}
                          </div>
                        </div>
                        ...
                        <div className="shrink-0"><Badge tone={statutTone(r.statut)} size="sm" dot>{statutLabel(r.statut)}</Badge></div>
                      </div>
                    );
                  })}
              </div>
