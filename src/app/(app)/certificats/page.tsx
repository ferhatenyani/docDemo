...
function label(t: TypeCertificat) {
  return t === "ARRET_TRAVAIL" ? "Arrêt de travail"
    : t === "MEDICAL" ? "Certificat médical"
      : t === "BON_TRANSPORT" ? "Bon de transport"
        : t === "ORIENTATION" ? "Lettre d'orientation"
          : t === "APTITUDE" ? "Certificat d'aptitude"
            : "Certificat";
}
...
export default function CertificatsListPage() {
  ...
  return (
    <div className="space-y-5">
      <SectionHeader
        eyebrow="Documents"
        title="Certificats & documents"
        description={`${certificats.length} documents émis`}
        actions={
          <Link href="/certificats/nouveau">
            <Button variant="dark" leftIcon={<Plus className="h-3.5 w-3.5" />}>Nouveau certificat</Button>
          </Link>
        }
      />
      ...
      <div className="bg-white rounded-lg border border-line shadow-xs">
        <div className="p-3 border-b border-line">
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Rechercher…" leftIcon={<Search className="h-3.5 w-3.5" />} />
        </div>

        {filtered.length === 0 ? (
          <EmptyState icon={<FileBadge className="h-5 w-5" />} title="Aucun certificat" action={<Link href="/certificats/nouveau"><Button variant="dark">Créer</Button></Link>} />
        ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {c.type === "ARRET_TRAVAIL" && c.duree_jours && (
                        <Badge tone="warning" size="sm">{c.duree_jours} j</Badge>
                      )}
                    </div>
                    <div className="text-[11px] text-ink-500 mt-0.5">{formatDate(c.date)} · {c.motif}</div>
                  </div>
